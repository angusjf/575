...
const signIn = async () => {
  ...
  const password = defineString("PASSWORD").value();
...
const notify = async (expoToken: string) => {
  const data = JSON.stringify({
    ...
    body: "Eurovision related haikus encouraged",
...
// cron('46 21 * * *',
export const sendNotifications = onSchedule("every day 16:22", async (req) => {
  let slackers = await getSlackers();
  slackers = [...new Set(slackers)];
